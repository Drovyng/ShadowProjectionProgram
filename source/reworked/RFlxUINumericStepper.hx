...
class RFlxUINumericStepper extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	private function set_value(f:Float):Float
	{
		...
		if (text_field != null)
		{
			...
			{
				displayValue *= 100;
				text_field.text = Std.string(decimalize(displayValue, decimals)) + "%" + textAdd;
			}
			else
			{
				text_field.text = decimalize(displayValue, decimals) + textAdd;
			}
		}
		return value;
	}
	...
	public function new(X:Float = 0, Y:Float = 0, StepSize:Float = 1, DefaultValue:Float = 0, Min:Float = -999, Max:Float = 999, Decimals:Int = 0,
			...
			IsPercent:Bool = false, ?TextAdd:String = "")
	{
		super(X, Y);

		if (TextField == null)
		{
			TextField = new FlxUIInputText(0, 0, 100);
		}
		...
		var size = text_field.size;
		text_field.height *= 2;
		text_field.size = size * 2;
		text_field.text = Std.string(DefaultValue) + textAdd;
